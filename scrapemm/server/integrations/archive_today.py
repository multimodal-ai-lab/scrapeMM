...
2. **Otherwise fetch the replay page** with the stored `cf_clearance`. The fetch client is
   ...
   too (see `BROWSER_TLS_DOMAINS` in `scrapemm.server.download.common`), so no session is needed.
...
from scrapemm.server.paths import (SNAPSHOT_CACHE_PATH, PAGE_CACHE_DIR,
                                   BUFFER_PATH)
...
GATE_HINT = (
    ...
    "scrapeMM does not solve captchas. The URL was buffered: open the server's web UI "
    "under CAPTCHA and solve one check in the panel there. Everything buffered so "
    "far is then retrieved and cached, so asking for it again succeeds without a "
    "session. Alternatively, the server setting archive_today_screenshot_fallback makes "
    "scrapeMM serve the snapshot's screenshot and metadata instead of failing."
)
...
class ArchiveToday(HeadedBrowser):
    ...
    async def probe(self) -> None:
        """Reports whether snapshot pages are actually reachable right now.

        The browser preconditions that `HeadedBrowser.probe()` checks are necessary but
        nowhere near sufficient here: Archive.today gates every replay page behind a
        CAPTCHA, and a solved session lasts about five minutes. So a card reading
        "Connected" between sessions would be actively misleading -- the honest answer
        comes from asking for a snapshot and seeing whether the gate is up.

        Costs one HTTP request, cached for the status TTL like every other probe.
        """
        await super().probe()  # Browser preconditions; raises if they do not hold

        url = f"https://{CANONICAL_DOMAIN}/{VERIFICATION_SNAPSHOT}"
        async with aiohttp.ClientSession() as session:
            status, body = await self._fetch_page(session, url)

        if status and 200 <= status < 300 and not _looks_like_gate(body):
            self.connected = True
            return

        # Cached pages stay retrievable without any session at all, so a gate is a
        # limitation rather than an outage -- which is what the caller is told.
        self.connected = False
        cached = len(_pages)
        raise CaptchaEncounteredError(
            f"Gated by the CAPTCHA right now, so new snapshots are buffered instead of "
            f"retrieved. {cached} already-cached page(s) remain available. Solve one "
            f"check in the CAPTCHA panel to clear the backlog.")
    ...
    async def _solve_in_browser(self, snapshot_url: str, timeout: float) -> bool:
        ...
        from scrapemm.server.integrations.headed_browser import _resolve_profile_dir
