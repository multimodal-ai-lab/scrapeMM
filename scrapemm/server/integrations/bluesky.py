...
class Bluesky(RetrievalIntegration):
    ...
    async def _still_connected(self) -> bool:
        """A logged-in atproto client carries its own profile. Checking for it is local,
        which matters: Bluesky rate-limits session creation hard, so the dashboard must
        never answer this question by logging in again."""
        return self.client is not None and getattr(self.client, "me", None) is not None
    ...
    async def _connect(self):
        ...
        # Re-use the logged-in client. Bluesky allows only a handful of createSession
        # calls per account per hour, and logging in on every status poll burned through
        # that allowance -- which is what "Could not connect" was actually reporting.
        if await self._still_connected():
            self.connected = True
            return
...
- Post count: {profile.posts_count}
    ...
    async def _authenticate(self) -> bool:
        ...
        try:
            ...
            # Saying so explicitly matters: leaving `connected` at None made the next
            # caller try to connect all over again, which is precisely what a rate
