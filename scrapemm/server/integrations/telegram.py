...
from scrapemm.server.paths import TELEGRAM_SESSION_PATH
...
class Telegram(RetrievalIntegration):
    ...
    async def _still_connected(self) -> bool:
        """Telethon knows locally whether its socket is up, so this costs nothing."""
    ...
    async def _connect(self):
        ...
        # An existing client owns the session file. Opening a second one on top of it is
        # what produced "database is locked", so the old one is closed first -- there is
        # never more than one client per session.
        await self._disconnect()

        try:
            self.client = TelegramClient(self.session_path, api_id, api_hash)
            await self.client.start(bot_token=bot_token)  # Coroutine: a loop exists already
        except sqlite3.OperationalError:
            # A previous run was interrupted and left its journal behind. Clearing it and
            # retrying once beats failing every retrieval until somebody deletes it.
            logger.warning("Telegram's session database was locked; clearing the stale "
                           "journal and retrying.", exc_info=True)
            await self._disconnect()
            Path(self.session_path + ".session-journal").unlink(missing_ok=True)
            self.client = TelegramClient(self.session_path, api_id, api_hash)
    ...
    async def _disconnect(self) -> None:
        """Releases the session file, best-effort: a client that is already gone is
        ...
        try:
            ...
            logger.debug("Could not disconnect the previous Telegram client.",
                         exc_info=True)
